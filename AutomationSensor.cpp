...
void AutomationSensor::update()
{
	...
	if(m_xModel)
	{
		m_xModel->setValue(qBound(m_xModel->getMin(),(float)qAbs (m_reading->x()*0.01*m_xModel->getMax() )
						   + m_xModel->getMin(), m_xModel->getMax() ));
	}
	if(m_yModel)
	{
		m_yModel->setValue(qBound(m_yModel->getMin(),(float)qAbs (m_reading->y()*0.01*m_yModel->getMax() )
						   + m_yModel->getMin(), m_yModel->getMax() ));
	}
	if(m_zModel)
	{
		m_zModel->setValue(qBound(m_zModel->getMin(),(float)qAbs (m_reading->z()*0.01*m_zModel->getMax() )
